...
# Generate build script dynamically inside the container
RUN cat <<'EOF' > /build/build.sh
#!/bin/bash
set -euo pipefail
echo "=== Costapine ISO Build starting ==="
...
for list in rootfs/packages/*.list; do
    ...
    pkgs=$(grep -v '^#' "$list" | grep -v '^$')
    if [ -n "$pkgs" ]; then
        apk --root "$ROOTFS_DIR" add $pkgs
    fi
done
